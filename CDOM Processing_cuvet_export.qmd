...
Horiba_file_path <- "Single_Cuvet_Data"
```
...
sample_info_path <- "sample_info_cuvet.xlsx"
...
```{r}
abs_data <- map_df(files_pct, ~read.delim(.x, sep = "\t",header = T) %>% 
                     filter(!row_number() %in% c(1,2)) %>%
                     mutate(File = basename(.x))) %>%
  separate(File,into = c("Sample_id",NA),sep = "_") %>%
  ...
  group_by(Sample_id) %>%
  mutate(abs_600 = Abs[which(Wavelength == max(Wavelength))],
