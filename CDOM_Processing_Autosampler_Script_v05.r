...
Sequence_Name <- "Schade_2024_Batch03" #This will be appended to your output file (.xlsx)

doc_worksheet <- "Schade_2024_Batch03_data_processed.xlsx"
doc_data <- read_excel(doc_worksheet,sheet = 1)

Horiba_file_path <- "Horiba DAT data"
...
abs_data <- map_df(files_pct, ~read.delim(.x, sep = "\t",header = F) %>% 
  ...
  group_by(Sample_id) %>%
  mutate(abs_baseline = Abs[which.min(abs(Wavelength - 500))],
         abs_corrected = Abs - abs_baseline)
         # abs_corrected = ifelse(abs_corrected < 0,0.00001,abs_corrected))
...
a254 <- 
  ...
  full_join(doc_data %>% filter(Analysis == "NPOC") %>%
            rename("Unique_id" = "Sample ID",
                   "Concentration_mg.L" = "Average concentration (mg/L)") %>%
...
### Plots based on absorbance data
...
a254_plot <-
  ...
  facet_wrap(~Group01,scales = "free") +
...
eem_data <- map_df(files_eem, ~read.delim(.x, sep = "\t",header = T) %>% 
  ...
  full_join(sample_info) %>%
  mutate(X = ifelse(X < 0,NA,X)) #Added to eliminate whitespace from EEM plots
...
HIX <-
  ...
  summarise(sum_intensity = sum(X,na.rm = T)) %>%
...
FI <- 
  ...
  mutate(FI = EM_470/(EM_520 + EM_520))
...
Sample_data_for_export <-
  a254 %>%
  full_join(slope_equation_parameters) %>%
  full_join(HIX) %>%
  full_join(BIX) %>%
  full_join(FI) %>%
  full_join(coble_ratios) %>%
  ungroup(.) %>%
  filter(Group01 == "Sample") %>%
  select(Unique_id,a254_m1,SUVA_254,S275_295,S350_400,Slope_ratio,HIX,BIX,FI,A,B,C,M,"T",A_to_T,C_to_A,C_to_M,C_to_T)
...
write_xlsx(list("Sample Data" = Sample_data_for_export,
                "QC Data" = QC_data_for_export,
                "Blank Data" = Blank_data_for_export),path = paste(Sequence_Name, "Processed_CDOM_data.xlsx", sep = "_"))
